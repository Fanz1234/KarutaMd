require('./src/config')
const { default: naimiConnect, useSingleFileAuthState, fetchLatestBaileysVersion, DisconnectReason, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
const fs = require('fs')
...
async function startnaimi() {
  ...
  const { version, isLatest } = await fetchLatestBaileysVersion()
	console.log(`VERSI WA v${version.join('.')}, isLatest: ${isLatest}`)
    console.log(color(figlet.textSync('KARUTA MD', {
		font: 'Standard',
		horizontalLayout: 'default',
		vertivalLayout: 'default',
		whitespaceBreak: false
	}), 'red'))
	console.log(color('\nADA SARAN ATAU MASUKAN WA : +62 877-4086-1813', 'yellow'))
...
let docs = pickRandom(documents)
    ...
    const isWelkom = welkom.includes(anu.id)
        console.log(anu)
      if (isWelkom){
        try {
...
naimi.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    naimi.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startnaimi(); } 
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startnaimi(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startnaimi(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startnaimi(); }
            else { console.log(`Unknown DisconnectReason: ${reason}|${connection}`); startnaimi(); }
        } else if(connection === 'open') {
            console.log('Bot conneted to server')
            naimi.sendMessage(setting.owner+'@s.whatsapp.net', { text: `Bot started!` })
        }
